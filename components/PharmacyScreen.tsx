...
const PharmacyScreen: React.FC<PharmacyScreenProps> = ({ onBack, aiManager }) => {
    ...
    return (
        <div className="flex flex-col h-full bg-slate-50">
            <div className="px-4 py-4 bg-teal-700 text-white rounded-b-[2.5rem] shadow-lg shrink-0 z-10">
                <div className="flex items-center justify-between mb-4">
                    <button onClick={onBack} className="w-10 h-10 rounded-full bg-white/20 flex items-center justify-center hover:bg-white/30 transition-colors backdrop-blur-md">
                        <ArrowLeftIcon />
                    </button>
                    <h1 className="text-xl font-bold tracking-tight">Smart Pharmacy</h1>
                    <div className="w-10 h-10 rounded-full bg-white/20 flex items-center justify-center backdrop-blur-md">
                        <PillIcon />
                    </div>
                </div>
                
                <p className="text-teal-100 text-xs font-medium mb-4 px-1">
                    AI Pharmacist • Indian Generics • Price Comparison
                </p>
                ...
                <div className="relative">
                    <input 
                        ...
                        placeholder="Describe symptoms (e.g., Fever, Cold...)"
                        className="w-full h-14 pl-12 pr-4 rounded-2xl bg-white text-slate-900 placeholder-slate-400 font-medium shadow-md focus:outline-none focus:ring-2 focus:ring-teal-300 transition-all"
                    />
                    ...
                    <button 
                        ...
                        className="absolute right-2 top-2 bottom-2 px-4 bg-teal-900 text-white rounded-xl text-xs font-bold hover:bg-teal-800 transition-colors"
                    >
                        {isLoading ? 'Searching...' : 'Find Meds'}
                    </button>
                </div>
            </div>
            ...
            <div className="flex-1 overflow-y-auto p-4">
                {isLoading && (
                    <div className="flex flex-col items-center justify-center h-64 space-y-4">
                        ...
                        <p className="text-slate-400 text-sm font-medium">Consulting Pharmacist Database...</p>
                    </div>
                )}
                ...
                {!isLoading && !result && (
                    <div className="flex flex-col items-center justify-center h-64 text-slate-400 opacity-60">
                        ...
                        <p className="text-sm">Enter symptoms to get prescriptions.</p>
                    </div>
                )}
                ...
                {result && (
                    <div className="space-y-6 animate-slideUpFade">
                        <div className="bg-white p-5 rounded-2xl border border-slate-100 shadow-sm">
                            <h3 className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-2">Assessment</h3>
                        ...
                        <div className="flex items-center justify-between px-1">
                            <h3 className="text-lg font-bold text-slate-900">Recommended Meds</h3>
                        ...
                        <div className="bg-blue-50 p-4 rounded-xl flex gap-3 items-start">
                            ...
                            <p className="text-xs text-blue-800 leading-relaxed">
                                <strong>Disclaimer:</strong> Prices are estimated market averages in India. Always verify with a pharmacist before purchasing. Use generic medicines (Jan Aushadhi) for cost savings with same efficacy.
                            </p>
                        </div>
                    </div>
                )}
